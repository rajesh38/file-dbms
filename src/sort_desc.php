  ...
  $attr_type=$_REQUEST['attr_type'];
	...
	while(file_get_contents($tab_data_file)!=="")
	{
		...
		foreach($arr as $record)
		{
			...
			if(!isset($max))
			{
				$max=substr($record, 0, strpos($record, $separator_string));
				$record_backup=$record_copy;
			}
			else
			{
				if(($attr_type=="char" && (strcasecmp( substr($record, 0, strpos($record, $separator_string)), $max )>0)) || ($attr_type=="number" && (substr($record, 0, strpos($record, $separator_string))>$max)))
				{
					$max=substr($record, 0, strpos($record, $separator_string));
					$record_backup=$record_copy;
				}
